...
import com.danilodps.pay.domain.model.response.TransferResponse;
...
public class KafkaEventProducer {
    ...
    public void publishKafkaDepositEventNotification(DepositResponse depositResponse){
        ...
        kafkaTemplate.send(KafkaConfig.DEPOSIT_NOTIFICATION_TOPIC, depositResponse.depositId(), depositResponse);
    }
    ...
    public void publishKafkaTransferEventNotification(TransferResponse transferResponse){
        ...
        kafkaTemplate.send(KafkaConfig.TRANSFER_NOTIFICATION_TOPIC, transferResponse.transferId(), transferResponse);
    }

    public void publishKafkaSignInNotification(SigninResponse signinResponse){
        log.info("Criação de usuário");
        kafkaTemplate.send(KafkaConfig.SIGN_IN_NOTIFICATION_TOPIC, signinResponse.id(), signinResponse);
    }

    public void publishKafkaSignUpNotification(SignupRequest signupRequest){
        log.info("Usuário logado");
        kafkaTemplate.send(KafkaConfig.SIGN_UP_NOTIFICATION_TOPIC, signupRequest.id(), signupRequest);
    }
}
